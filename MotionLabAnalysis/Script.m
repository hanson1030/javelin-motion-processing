...
%% -------------------------------------------------------------------------
% Import data
if(max(size(left_data.accelerometer_data.data))<max(size(left_data.gyroscope_data.data)))
    time = zeros(size(left_data.accelerometer_data.data));
else
    time = zeros(size(left_data.gyroscope_data.data));
end
%samplingRate = data.accelerometer_data.sampling_rate;
samplingRate=100;
samplePeriod = 1/samplingRate;
[m,length] = size(time);
[gyrXL,gyrYL,gyrZL,gyrXR,gyrYR,gyrZR] = deal(zeros(size(time)));
[accXL,accYL,accZL,accXR,accYR,accZR] = deal(zeros(size(time)));

for i=1:length
    time(i)=(samplePeriod)*i;
end

for i=1:length
    
accXL(i) = left_data.accelerometer_data.data{1,i}{1,1}/2;
accYL(i) = left_data.accelerometer_data.data{1,i}{1,2}/2;
accZL(i) = left_data.accelerometer_data.data{1,i}{1,3}/2;
gyrXL(i) = left_data.gyroscope_data.data{1,i}{1,1};
gyrYL(i) = left_data.gyroscope_data.data{1,i}{1,2};
gyrZL(i) = left_data.gyroscope_data.data{1,i}{1,3};
accXR(i) = right_data.accelerometer_data.data{1,i}{1,1}/2;
accYR(i) = right_data.accelerometer_data.data{1,i}{1,2}/2;
accZR(i) = right_data.accelerometer_data.data{1,i}{1,3}/2;
gyrXR(i) = right_data.gyroscope_data.data{1,i}{1,1};
gyrYR(i) = right_data.gyroscope_data.data{1,i}{1,2};
gyrZR(i) = right_data.gyroscope_data.data{1,i}{1,3};

end

time=transpose(time);
accXL=transpose(accXL);
accYL=transpose(accYL);
accZL=transpose(accZL);
gyrXL=transpose(gyrXL);
gyrYL=transpose(gyrYL);
gyrZL=transpose(gyrZL);
accXR=transpose(accXR);
accYR=transpose(accYR);
accZR=transpose(accZR);
gyrXR=transpose(gyrXR);
gyrYR=transpose(gyrYR);
gyrZR=transpose(gyrZR);
...
accL_mag = sqrt(accXL.*accXL + accYL.*accYL + accZL.*accZL);
accR_mag = sqrt(accXR.*accXR + accYR.*accYR + accZR.*accZR);

% HP filter accelerometer data
filtCutOff = 0.001;
[b, a] = butter(1, (2*filtCutOff)/(1/samplePeriod), 'high');
accL_magFilt = filtfilt(b, a, accL_mag);
accR_magFilt = filtfilt(b, a, accR_mag);
% Compute absolute value
accL_magFilt = abs(accL_magFilt);
accR_magFilt = abs(accR_magFilt);
% LP filter accelerometer data
filtCutOff = 5;
[b, a] = butter(1, (2*filtCutOff)/(1/samplePeriod), 'low');
accR_magFilt = filtfilt(b, a, accR_magFilt);
accL_magFilt = filtfilt(b, a, accL_magFilt);
...
%                 'Position', [9 39 1280 768], 'View', [(100:(Spin/(max(size(posPlot)-1))):(100+Spin))', 10*ones(max(size(posPlot)), 1)], ...
