...
startTime = 0;
stopTime = max(size(time))*samplePeriod;

[m,length] = size(time);
[gyrXL,gyrYL,gyrZL] = deal(zeros(size(time)));
[accXL,accYL,accZL] = deal(zeros(size(time)));
...
accXL(i) = left_data.accelerometer_data.data{1,i}{1,1};
accYL(i) = left_data.accelerometer_data.data{1,i}{1,2};
accZL(i) = left_data.accelerometer_data.data{1,i}{1,3};
...
%accL_mag = sqrt(accXL.*accXL+accYL.*accYL);
accL_magZ = accZL;
accL_magY = accYL;
accL_magX = accXL;
...
accL_magFiltZ = filtfilt(b, a, accL_magZ);
accL_magFiltY = filtfilt(b, a, accL_magY);
accL_magFiltX = filtfilt(b, a, accL_magX);
...
accL_magFiltZ = filtfilt(b, a, accL_magFiltZ);
accL_magFiltY = filtfilt(b, a, accL_magFiltY);
accL_magFiltX = filtfilt(b, a, accL_magFiltX);
...
ax(1) = subplot(2,1,1);
    hold on;
    plot(time,accL_magFiltX,'r');
    plot(time,accL_magFiltY,'g');
    plot(time,accL_magFiltZ,'b');
...
ax(2) = subplot(2,1,2);
    hold on;
    plot(time,sqrt(accL_magFiltX.*accL_magFiltX+accL_magFiltY.*accL_magFiltY)-accL_magFiltZ);
    title('Stroke Count Signal');
    xlabel('Time (s)');
    ylabel('Magnitude');
    hold off;
linkaxes(ax,'x');
